    ...
    <div class="min-h-screen w-full bg-gradient-to-br from-[#0b0f1f] to-[#1c2236] text-gray-200 px-6 py-10">
      <div class="max-w-5xl mx-auto">
        ...
        <div class="flex items-center justify-between mb-6">
          <h1 class="text-3xl font-extrabold">${t.stats_pong_title}</h1>
            ...
            <table class="min-w-full text-sm">
              <thead class="text-slate-400">
                <tr class="border-b border-slate-700/40">
                  ...
                  <th class="text-left py-3 pr-4">${t.score}</th>
                  <th class="text-left py-3 pr-2">${t.result}</th>
                </tr>
              </thead>

              <tbody id="history-body" class="text-slate-200">
                <tr>
                  <td class="py-4 text-slate-500" colspan="4">${t.loading}</td>
                </tr>
              </tbody>
            </table>
          </div>
    ...
    try {
      ...
      tbody.innerHTML = matches
        .map((m) => {
          ...
          const resultText = isWin ? t.win : t.loss;
